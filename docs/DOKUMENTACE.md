...
### 5.4 Příručka pro návštěvníka webu
...
4. **Nahrání souborů přes FTP** – Připojte se k FTP serveru Wedos pomocí klienta (např. FileZilla [doporučeno]). FTP údaje najdete v klientské zóně Wedos. Nahrajte celou složku WordPressu do kořenového adresáře webu (`www/`).
...
Kromě Wedos lze web nasadit na jakýkoli hosting s podporou PHP ≥ 8.0 a MySQL/MariaDB (např. Active24, Blueboard). Postup je shodný – liší se pouze přístup ke klientské zóně a konkrétní nastavení FTP a databáze. Pro lokální vývoj je doporučen XAMPP (viz sekce 5.1.2).
