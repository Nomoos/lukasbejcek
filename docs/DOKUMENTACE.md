...
Celý projekt je dostupný jako open-source repozitář na GitHubu [1] a je připraven k lokálnímu nasazení pomocí XAMPP nebo wp-env.
...
Na základě zadání maturitní práce [2] a potřeb klubu byly stanoveny tyto požadavky:
...
Pro implementaci byl zvolen redakční systém WordPress [3], nejrozšířenější CMS na světě s podílem přes 40 % všech webových stránek. WordPress nabízí:
...
- Rozsáhlou dokumentaci a komunitu [4]

#### 2.3.2 CSS framework – Bootstrap 5

Pro responzivní layout byl zvolen framework Bootstrap 5 [5], který poskytuje:
...
Bootstrap je načítán z CDN [6], čímž odpadá nutnost build nástrojů a zároveň se využívá cache prohlížeče.

#### 2.3.3 Lokální vývojové prostředí – XAMPP

Pro lokální vývoj a testování byl zvolen balík XAMPP [7], který obsahuje Apache webserver, MySQL databázi a PHP interpret v jedné instalaci. XAMPP je dostupný zdarma pro Windows, macOS i Linux.

#### 2.3.4 Grafický návrh – Figma

Grafický návrh webu byl vytvořen v nástroji Figma [8] ve variantách pro desktop i mobilní zařízení.

### 2.4 Vlastní implementace vs. externí pluginy

Zadání práce doporučuje řadu externích pluginů: Custom Post Type UI [9] pro registraci CPT, Advanced Custom Fields [10] pro správu vlastních polí, FacetWP pro filtrování obsahu, The Events Calendar pro kalendář zápasů, Modula pro galerie a User Role Editor pro správu rolí.
...
Pro tento projekt byly zvoleny CPT z následujících důvodů [4]:
...
Pro zobrazení seznamů CPT (zápasy, týmy, galerie) byly zvoleny **archivní šablony** (`archive-zapas.php`) místo stránkových šablon (`page-zapasy.php`). Důvody [4]:
...
Toto rozdělení odpovídá doporučené WordPress architektuře [4]: téma řeší „jak vypadá", plugin řeší „jak se spravuje".
...
Projekt definuje šest vlastních typů obsahu registrovaných v `functions.php` pomocí funkce `register_post_type()` [4]:
...
Projekt používá čtyři vlastní taxonomie registrované přes `register_taxonomy()` [4]:
...
Místo pluginu Advanced Custom Fields [10] jsou meta boxy implementovány přímo v PHP pomocí WordPress funkcí `add_meta_box()`, `get_post_meta()` a `update_post_meta()` [4].
...
Místo pluginů FacetWP nebo Search & Filter Pro [11] jsou filtry implementovány jako HTML formuláře s GET parametry:
...
PHP kód v šabloně načte GET parametry, sanitizuje je pomocí `sanitize_text_field()` a předá do `WP_Query` s `tax_query` a `meta_query` [4].
...
Styly jsou rozdělené do komponentových souborů a načítány přes `wp_enqueue_style()` [4] s definovanými závislostmi:
...
Soubor `style.css` v kořeni tématu slouží pouze jako identifikátor pro WordPress (název tématu, verze, autor) a neobsahuje žádné styly. Veškeré styly a skripty jsou načítány přes `wp_enqueue_style()` a `wp_enqueue_script()` – nikdy přímo v šablonách [4].
...
Místo pluginu User Role Editor [12] je vlastní role registrována přímo v kódu:
...
|---|---|
| Lazy loading obrázků | Funkce `slavoj_add_lazy_loading()` přidává `loading="lazy"` [13] |
| Bootstrap z CDN | CSS a JS z jsdelivr.net – využívá cache prohlížeče [6] |
...
Projekt dodržuje standardní bezpečnostní praktiky WordPressu [4] [14]:
...
Projekt záměrně minimalizuje použití JavaScriptu. Celkový rozsah vlastního JS je přibližně 70 řádků [15]:
...
4. **Kontrola WordPress best practices** – dodržení standardů pro vývoj témat [14]
...
Kontrola kódu provedená v březnu 2026 potvrdila dodržení bezpečnostních standardů [14]:
...
- XAMPP [7] (Apache + MySQL + PHP 8.x) nebo jiný webový server s PHP a MySQL
- Git [16] pro klonování repozitáře

#### 5.1.2 Postup instalace (XAMPP)

1. Nainstalujte a spusťte XAMPP – zapněte Apache a MySQL.

2. Stáhněte WordPress [3] do `C:\xampp\htdocs\fotbal_club\`.
...
Pro vývojáře se znalostí Dockeru [17] je k dispozici konfigurace `.wp-env.json`:
...
1. **Kalendářní modul** – vizuální kalendář zápasů (doporučen plugin The Events Calendar [18])
2. **SEO optimalizace** – integrace pluginu Rank Math [19] nebo Yoast SEO [20]
3. **Cache a komprese** – nasazení WP Super Cache [21] a Smush [22] pro produkční prostředí
...
[1] BEJČEK, Lukáš. *lukasbejcek: Webová prezentace TJ Slavoj Mýto* [software]. GitHub [online]. 2026 [cit. 2026-03-27]. Dostupné z: https://github.com/Nomoos/lukasbejcek

[2] BEJČEK, Lukáš. *Zadání maturitní práce: Webová prezentace fotbalového klubu s databází zápasů a správou obsahu ve WordPressu* [dokument]. 2025. Součást repozitáře [1], soubor `ZADANI-MATURITNI-PRACE.md`.

[3] WORDPRESS FOUNDATION. *WordPress* [software]. Verze 6.x. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://wordpress.org

[4] WORDPRESS FOUNDATION. *WordPress Developer Resources* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://developer.wordpress.org

[5] OTTO, Mark a Jacob THORNTON. *Bootstrap* [software]. Verze 5.3.3. [b.m.]: Bootstrap Team, 2024 [cit. 2026-03-27]. Dostupné z: https://getbootstrap.com

[6] JSDELIVR. *jsDelivr – free CDN for open source* [online]. 2024 [cit. 2026-03-27]. Dostupné z: https://www.jsdelivr.com

[7] APACHE FRIENDS. *XAMPP* [software]. [b.m.]: Apache Friends, 2024 [cit. 2026-03-27]. Dostupné z: https://www.apachefriends.org

[8] FIGMA, INC. *Figma* [software]. San Francisco (CA): Figma, 2024 [cit. 2026-03-27]. Dostupné z: https://www.figma.com

[9] FLAVOR, WebDevStudios. Custom Post Type UI. In: *WordPress Plugin Directory* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://wordpress.org/plugins/custom-post-type-ui/

[10] WP ENGINE. Advanced Custom Fields. In: *WordPress Plugin Directory* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://wordpress.org/plugins/advanced-custom-fields/

[11] FACETWP, LLC. *FacetWP – Advanced Filtering for WordPress* [online]. 2024 [cit. 2026-03-27]. Dostupné z: https://facetwp.com

[12] FLAVOR, Vladimir Garagulja. User Role Editor. In: *WordPress Plugin Directory* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://wordpress.org/plugins/user-role-editor/

[13] WORLD WIDE WEB CONSORTIUM (W3C). *HTML Living Standard: Lazy loading* [online]. 2024 [cit. 2026-03-27]. Dostupné z: https://html.spec.whatwg.org/multipage/urls-and-fetching.html#lazy-loading-attributes

[14] WORDPRESS FOUNDATION. *Theme Developer Handbook* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://developer.wordpress.org/themes/

[15] BEJČEK, Lukáš. *JavaScript v projektu TJ Slavoj Mýto* [dokument]. 2026. Součást repozitáře [1], soubor `docs/pracovni/12-javascript.md`.

[16] SOFTWARE FREEDOM CONSERVANCY. *Git* [software]. Verze 2.x. [b.m.]: Software Freedom Conservancy, 2024 [cit. 2026-03-27]. Dostupné z: https://git-scm.com

[17] DOCKER, INC. *Docker* [software]. San Francisco (CA): Docker, 2024 [cit. 2026-03-27]. Dostupné z: https://www.docker.com

[18] THE EVENTS CALENDAR. The Events Calendar. In: *WordPress Plugin Directory* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://wordpress.org/plugins/the-events-calendar/

[19] FLAVOR, Starter Sites. Starter Templates. Rank Math SEO. In: *WordPress Plugin Directory* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://wordpress.org/plugins/seo-by-rank-math/

[20] YOAST BV. Yoast SEO. In: *WordPress Plugin Directory* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://wordpress.org/plugins/wordpress-seo/

[21] FLAVOR, Developer. WP Super Cache. In: *WordPress Plugin Directory* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://wordpress.org/plugins/wp-super-cache/

[22] FLAVOR, Developer. Smush. In: *WordPress Plugin Directory* [online]. San Francisco (CA): WordPress Foundation, 2024 [cit. 2026-03-27]. Dostupné z: https://wordpress.org/plugins/wp-smushit/

---

## Příloha A – Kopie zadání práce

Viz soubor `ZADANI-MATURITNI-PRACE.md` v kořeni repozitáře [1].

## Příloha B – Přístupové údaje

Viz soubor `PRISTUPOVE-UDAJE.md` v kořeni repozitáře [1].
