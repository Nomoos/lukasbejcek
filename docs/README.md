# docs/ – Dokumentace projektu TJ Slavoj Mýto

Tato složka obsahuje průběžnou dokumentaci portování WordPress webu TJ Slavoj Mýto.

## Obsah

| Soubor | Popis |
|--------|-------|
| [01-uvod.md](./01-uvod.md) | Úvod a přehled projektu |
| [02-analyza-original.md](./02-analyza-original.md) | Analýza původního kódu ve složce `original/` |
| [03-nova-struktura.md](./03-nova-struktura.md) | Nová adresářová a datová struktura |
| [04-lokalni-instalace.md](./04-lokalni-instalace.md) | Jak rozchodit projekt lokálně (Local, XAMPP, Docker) |
| [05-deployment-ftp.md](./05-deployment-ftp.md) | Deployment na hosting přes FTP |
| [06-inventar-obsahu.md](./06-inventar-obsahu.md) | Inventář všech typů obsahu (CPT) s poli – Fáze 1 |

## Jak přispívat

- Každou dokončenou fázi zaznamenejte do příslušného souboru.
- Pro nová témata vytvořte nový soubor s číselným prefixem (např. `04-sablony.md`).
- Udržujte dokumenty stručné a aktuální.
